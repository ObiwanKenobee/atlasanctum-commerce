...
export class CheckoutPage implements OnInit {
      ...
      .then(
        (data: any) => {
          ...
          if (data && data.status == 200 && data.data.length) {
            ...
            this.util.showToast(
              this.util.translate('No Stores Found'),
              'danger',
              'bottom'
            );
            this.util.onBack();
  ...
  async payWithFlutterwave() {
    ...
    browser.on('loadstop').subscribe((event: any) => {
      ...
      if (
        navUrl.includes('success_payments') ||
        navUrl.includes('failed_payments')
      ) {
        console.log('close');
        browser.close();
        ...
        (data: any) => {
          ...
          if (
            ...
            const browser: any = this.iab.create(
              data.success.payment_request.longurl,
              '_blank',
              options
            );
            browser.on('loadstop').subscribe((event: any) => {
  ...
  async payWithRazorPay() {
    ...
    const param = {
      ...
      name: this.getName(),
      app_color:
        this.util && this.util.app_color ? this.util.app_color : '#f47878',
    };
    const browser = this.iab.create(
      this.api.baseUrl +
      'v1/payments/razorPay?' +
      this.api.JSON_to_URLEncoded(param),
      '_blank',
      options
    );
    ...
    browser.on('loadstop').subscribe((event) => {
      ...
      if (
        navUrl.includes('status=authorized') ||
        navUrl.includes('status=failed')
      ) {
        ...
        console.log(
          '--->>',
          urlItems.split('/'),
          urlItems.split('/').length,
          urlItems.split('/')[3]
        );
        if (
          urlItems.split('/').length > 5 &&
          urlItems.split('/')[3].startsWith('pay_')
        ) {
  ...
  async payWithPayTm() {
    ...
    const browser = this.iab.create(
      this.api.baseUrl + 'v1/payNow?amount=' + this.cart.grandTotal,
      '_blank',
      options
    );
    console.log('opended');
  ...
  async payWithPayPal() {
    ...
    const browser = this.iab.create(
      this.api.baseUrl + 'v1/payments/payPalPay?amount=' + this.cart.grandTotal,
      '_blank',
      options
    );
    console.log('opended');
  ...
  async stripePayment() {
    ...
    const amountToPay = this.cart.grandTotal * 100;
    const browser = this.iab.create(
      this.api.baseUrl + 'v1/payments/stripeAppCheckout?amount=' + amountToPay,
      '_blank',
      options
    );
    console.log('opended');
